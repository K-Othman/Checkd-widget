...
const FHShotsOnTarget = () => {
  ...
  const homeStat = matchesDetails.match.liveData.lineups.home.stats.find(
    (stat) => stat.type === "ontargetScoringAtt" && stat.fh
  );
  ...
  const homeOnTargetShots = Number(homeStat.fh);
  const awayOnTargetShots = Number(awayStat.fh);
...
export default FHShotsOnTarget;
