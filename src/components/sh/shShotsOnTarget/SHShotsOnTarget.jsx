...
const SHShotsOnTarget = () => {
  ...
  const homeStat = matchesDetails.match.liveData.lineups.home.stats.find(
    (stat) => stat.type === "ontargetScoringAtt" && stat.sh
  );
  ...
  const homeOnTargetShots = Number(homeStat.sh);
  const awayOnTargetShots = Number(awayStat.sh);
...
export default SHShotsOnTarget;
